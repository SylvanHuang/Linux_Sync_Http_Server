 ...
 */
static int handleFileRequest(char* webRoot,const char *requestURI, int clntSock)
{   

    fprintf(stderr,"in handlefilerequest\n");
    int statusCode;
    FILE *fp = NULL;
    ...
    fprintf(stderr,"fd opened for read\n");
...
void * threadMain( void *tparam){
    ...
    int clntSock=param -> clntSocket;
    struct sockaddr_in clntAddr;
    char * saveptr;


        FILE *clntFp = fdopen(clntSock, "r");
        fprintf(stderr,"threadmain fdopen() pass\n");
        if (clntFp == NULL)
            die("fdopen failed");
...
int main(int argc, char *argv[])
{
    ...
    int servSock = createServerSocket(servPort);
        ...
        if (clntSock < 0)
            die("accept() failed");
        param.clntSocket = clntSock;
        fprintf(stderr,"in for(;;) before pthread init\n");
        pthread_t tid;
        pthread_create(&tid,NULL,&threadMain,&param);
