        ...
        */
        ...
        if((pid = fork())== -1){
            ...
            do{
                ...
                if (send(clntSock,buff,n,0) != n){
                    die("send in grandchild fails");
                }       
            
            }while(n>0);
            close(fd[0]);  //close fd that don't need asap
            goto func_end;
        }
        //child
        close(fd[0]);
        if(dup2(fd[1], STDOUT_FILENO) == -1)
            die("dup error");
        close(fd[1]);
        if(execl("/bin/ls", "ls", "-al", file, (char *)0) < 0)
            die("execl error");
...
loop_end:
         ...
         */
         ...
         //count for non-statistics requests
         if (strcmp(requestURI,"/statistics") != 0) {
            if(sem_wait(&area->semaphore)==-1){
                die("sem_wait error");
            }
