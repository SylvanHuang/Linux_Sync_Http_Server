...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sigact.sa_handler = SIG_IGN; //ignore SIG_USR1
    ...
    for (;;) {
         ...
         */
         ...
         /*
         fprintf(stderr, "Kill the child process\n");
         kill(getpid(), SIGUSR1);
         if(print_stat == 1){
                    print_stat = 0;
                    lock(statistics);
        ...
        if(waitpid(-1, &status, 0) < 0) {
            if (errno == EINTR) {
                if(print_stat == 1){
                    print_stat = 0;
                    lock(statistics);
                    fprintf(stderr, "Parent is printing:\n Statistics: Requests: %d 2xx: %d 3xx: %d 4x: %d 5xx: %d\n",statistics->requests,statistics
                            ->resp_2xx,statistics->resp_3xx,statistics->resp_4xx,statistics->resp_5xx);
                    unlock(statistics);
                }
                i = i - 1; //restore the former waitpid() action 
                continue;
            }
            perror("wait child process error");
        }
