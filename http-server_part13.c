...
static int sock_array[N_THREADS][2];
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    pid_t pID;
    char buf[100]; // for ip address
    char line[1000];
    char requestLine[1000];
    int statusCode;
    struct sockaddr_in clntAddr;
    int index, i; // act as index for child process

    for(i = 0; i < N_THREADS; ++i){
        index = i;
        if(socketpair(AF_UNIX, SOCK_STREAM, 0, sock_array[i]) != 0)
    ...
    if(pID > 0){
        ...
        for (;;){
            ...
            sendConnection(clntSock, sock_array[cur][0]);
            close(clntSock);
            cur++;
            if(cur == 16) cur = 0;
    ...
    for (;;) {
        ...
        unsigned int clntLen = sizeof(clntAddr); 
        int clntSock = recvConnection(sock_array[index][1]);
        ...
        */
        if (getpeername(clntSock,(struct sockaddr *)&clntAddr, &clntLen) !=0 ){
            perror("getpeername fails");
        }
        inet_ntop(AF_INET, &clntAddr.sin_addr,buf,clntLen);
